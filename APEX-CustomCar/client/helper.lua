...
local function getUiValueLabel(groupName, value)
    local uiLabels = (Config and Config.UILabels) or {}
    if type(uiLabels) ~= 'table' then return value end

    local valueLabels = uiLabels.ValueLabels
    if type(valueLabels) ~= 'table' then return value end

    local group = valueLabels[groupName]
    if type(group) ~= 'table' then return value end

    return group[value] or value
end
...
function GetVehicleModIndexLabel(vehicle, modType, data)
	...
	if (data == -1) then
		if (not (type(modType) == 'number' and modType >= 17 and modType <= 22)) then
			return getUiValueLabel('default', defaultLabel or 'Default')
		end
	end
	...
	elseif (type(modType) == 'number' and modType >= 11 and modType <= 16 and modType ~= 14) then
		return getUiValueLabel('levelPrefix', levelPrefixLabel or 'Level') .. ' ' .. (data + 1)
	elseif (modType == 'extras') then
		return getUiValueLabel('extraPrefix', extraPrefixLabel or 'Extra') .. ' ' .. (data + 1)
	elseif (modType == 'modXenon') then
		local label = modXenonLabel[data + 1] or nil
		return getUiValueLabel('modXenonLabel', label)
	elseif (type(modType) == 'number' and modType >= 17 and modType <= 22) then
		local tempLabel = modOnOff or { 'OFF', 'ON' }
		local label = tempLabel[data + 2]
		return getUiValueLabel('modOnOff', label)
	elseif (type(modType) == 'number') then
